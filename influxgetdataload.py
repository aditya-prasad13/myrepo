...
class Influxgetdataload(restful.Resource):
        ...
        try:
            if not self.content['start_time'].encode('utf8') or not self.content['end_time'].encode('utf8'):
                ...
                while j<len(result):
                  ...
                  first = [result[j]['time']*1000, result[j]['load1']]
                  second = [result[j]['time']*1000, result[j]['load5']]
                  third = [result[j]['time']*1000, result[j]['load15']]
                  #return first
                  #return second
                  my_stats_1.append(first)
                  my_stats_2.append(second)
                  my_stats_3.append(third)
                  j=j+1
                row_value1 = json.dumps(my_stats_1)
                #return row_value1
                row_value2 = json.dumps(my_stats_2)
                row_value3 = json.dumps(my_stats_3)
                final = dict([('load1', row_value1), ('load5', row_value2), ('load15', row_value3)])
