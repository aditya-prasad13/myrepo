...
class Influxgetdataswp(restful.Resource):
	...
	host = host.replace(" ", "")
		...
		data = self.client.query(default_query,epoch='s' )
                ...
                while j<len(result):
                  ...
                  first = [result[j]['time']*1000, result[j]['free']/1073741824]
                  second = [result[j]['time']*1000, result[j]['used']/1073741824]
                  #return first
                  #return second
                ...
                final = dict([('free', row_value1), ('used', row_value2)])
